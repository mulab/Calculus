...
(*The integrand is not a rational function and possesses the form ax^r*(c1+c2*x^q)*p,
where A,c1,c2 are constants,p,q,r are rational numbers and c1*c2*q*p!=0*)
intSubBin[f_,x_]:=Module[
	{e=f,l,part,i,se,se1,se2,a=1,r,p,q,c1,c2,r1,r2,d,y},
	If[Head[e]===Times,
		l=Length[e];
		...
		If[Head[r1]===Integer&&r1<0&&Head[r2]===Rational,(*substitute y=(c1+c2*x^q)^(1/d2),d2 is the denominator of r2*)
			...
			e=e/.x->((y^d-c1)/c2)^(1/q);
			Return[{Simplify[e],y,(c1+c2*x^q)^(1/d)}]
		];
		If[Head[r1+r2]===Integer,(*substitute z=((c1+c2*y)/y)^(1/d1),d1 is the denominator of r1*)
			d=Denominator[r1];
			e=e/((-q*c1/d)*(c1/x^q+c2)^(1/d-1)/x^(q+1));
			e=e/.x->(c1/(y^d-c2))^(1/q);
			Return[{Simplify[e,y>0],y,((c1+c2*x^q)/(x^q))^(1/d)}]
		],
		Return["NotMatch"]
	]
]
